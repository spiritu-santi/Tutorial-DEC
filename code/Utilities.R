### Utilities
### These functions are mostly unaltered from Landis et al.
...
make_vib_clade_mtx = function(phy) {
  ...
  clade_mtx = rbind( clade_mtx, c( "Dicksoniaceae",    getMRCA(phy, c("Lophosoria_quadripinnata","Dicksonia_antarctica")),       2, 1 ))
  clade_mtx = rbind( clade_mtx, c( "Cyatheaceae",        getMRCA(phy, c("Cyathea_cranhamii","Alsophila_serratifolia")),       2, 1 ))
  clade_mtx = rbind( clade_mtx, c( "Culcitaceae",     getMRCA(phy, c("Culcita_conniifolia","Culcita_emberi")),       2, 1 ))
  clade_mtx = rbind( clade_mtx, c( "Cibotiaceae",      getMRCA(phy, c("Cibotium_iwatense","Cibotium_chamissoi")),     2, 1 ))
  clade_mtx = rbind( clade_mtx, c( "Plagiogyriaceae",        getMRCA(phy, c("Plagiogyria_decrescens","Plagiogyria_adnata")),              2, 1 ))
  clade_mtx = rbind( clade_mtx, c( "Metaxyaceae",   getMRCA(phy, c("Metaxya_elongata","Metaxya_lanosa")),        2, 1 ))
  clade_mtx = rbind( clade_mtx, c( "Loxomataceae",       getMRCA(phy, c("Loxsomopteris_anasilla","Loxoma_cunninghamii")),       2, 1 ))
  clade_mtx = rbind( clade_mtx, c( "Thyrsopteridaceae",       getMRCA(phy, c("Thyrsopteris_cretaceae","Thyrsopteris_elegans")),       2, 1 ))
  clade_mtx = rbind( clade_mtx, c( "Cyathea",    getMRCA(phy, c("Cyathea_bicrenata","Cyathea_sp_Graham")),            2, 1 ))
  clade_mtx = rbind( clade_mtx, c( "Alsophila",         getMRCA(phy, c("Alsophila_acutula","Alsophila_stelligera")),             2, 1 ))
  clade_mtx = rbind( clade_mtx, c( "Gymnosphaera",   getMRCA(phy, c("Gymnosphaera_impolita","Gymnosphaera_capensis")),         2, 1 ))
  clade_mtx = rbind( clade_mtx, c( "Sphaeropteris",   getMRCA(phy, c("Sphaeropteris_aeneifolia","Sphaeropteris_novaecaledoniae")),      2, 1 ))
